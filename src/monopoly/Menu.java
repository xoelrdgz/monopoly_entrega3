...
public class Menu {
    ...
    private int jugadormasvueltas;//Hechooooo
    ...
    public void iniciarPartida() {
        ...
        while (true) {
            ...
            if(jugadores.get(turno1).isEnCarcel()){jugadores.get(turno1).setCocheExtra(false);}
            if(jugadores.get(turno1).getCocheExtra()){System.out.println(jugadores.get(turno1).getNombre()+" lleva "+jugadores.get(turno1).turnoscocheavanzado+" turnos extra de los 3 posibles");jugadores.get(turno1).turnoscocheavanzado++;turno=turno-1;if(turno<0){turno=jugadores.size()+turno;}
                if(jugadores.get(turno).turnoscocheavanzado==3){System.out.println("Has llegado a 3 turnos del modo avanzado, este es el último");jugadores.get(turno).turnoscocheavanzado=0;jugadores.get(turno).setCocheExtra(false);}}
            if(jugadores.get(turno).getCocheProhibido()){System.out.println("Nos saltamos el turno de "+ jugadores.get(turno).getNombre()+" debido a que sacó menor a 4");
                jugadores.get(turno).turnossinjugarcocheavanzado++;turno++;
                if(jugadores.get(turno).turnossinjugarcocheavanzado==2){System.out.println("Ya han pasado los 2 turnos sin jugar, al siguiente ya juegas");jugadores.get(turno).turnossinjugarcocheavanzado=0;jugadores.get(turno).setCocheProhibido(false);}}
            ...
            if(jugadores.get(turno).isEnCarcel()){setTirado(true); salirCarcel();}
                ...
                String nuevoComando = scanner.nextLine();  // Cambié el nombre de la variable
...
*/
    ...
    private void salirCarcel() {
        ...
        if (turnosEnCarcel == 3) {if (jugadorActual.getFortuna() >= SUMA_VUELTA / 4) {
    ...
    public void incrementarTurnosEnCarcel() {
        if (jugadores.get(turno).isEnCarcel()) {
            turnosEnCarcel++;  // Incrementar si está en la cárcel
        }
    }
            ...
            case 6:
                ...
                for (int i = 0; i < jugadores.size(); i++) {  // Itera sobre todos los jugadores
                    if (i != turno) {  // Omite el jugador en turno
                        jugadores.get(i).sumarFortuna(cantidad);jugadores.get(i).cobroalquileres=jugadores.get(i).cobroalquileres+cantidad;  // Realiza el pago
                    }
                }

                jugador.sumarFortuna(-cantidad * (jugadores.size() - 1));jugador.pagoalquileres=jugador.pagoalquileres+(cantidad * (jugadores.size() - 1));  // Resta el total pagado
    ...
    public void movercocheavanzadomenor4(Jugador jugador, int valorTirada, int posicionActual) {
        ...
        if (nuevaPosicion > 39 && posicionActual <= 39) {
            ...
            // Verificar si todos los jugadores han completado al menos 4 vueltas
            if (todosHanCompletadoCuatroVueltas()) {
                Menu.tablero.incrementarPrecioSolares();  // Incrementar el precio de los solares en un 5%
                System.out.println("Todos los jugadores han dado 4 vueltas. El precio de los solares ha aumentado un 5%.");
            }
        }
        // Ajustar la nueva posición en el tablero si excede el número de casillas
        ...
        jugador.setCocheProhibido(true);jugador.setCocheExtra(false);
